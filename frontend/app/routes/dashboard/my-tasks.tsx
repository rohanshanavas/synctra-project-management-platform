...
import { ArrowUpNarrowWide, ArrowUpRight, CheckCircle, Clock, FilterIcon } from 'lucide-react';
...
const MyTasks = () => {
    ...
    const [sort, setSort] = useState<"asc" | "desc">(initialSort === "asc" ? "asc" : "desc");
    ...
    const filteredTasks = myTasks?.length > 0 ? myTasks.filter((task) => {
    ...
    const sortedTasks = [...filteredTasks].sort((a, b) => {
        if (a.dueDate && b.dueDate) {
            return sort === "asc" ? new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime() : new Date(b.dueDate).getTime() - new Date(a.dueDate).getTime();
        }
        return 0;
    });
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-start md:items-center justify-between">
                ...
                <div className="flex flex-col items-start md:flex-row md:items-center gap-2">
                    <Button variant="outline" onClick={() => setSort(sort === "asc" ? "desc" : "asc")}>
                        <ArrowUpNarrowWide className={`h-4 w-4 transition-transform ${sort === "asc" ? "" : "rotate-180"}`} />
            ...
            <Input className="max-w-md" placeholder="Search tasks..." value={search} onChange={(e) => setSearch(e.target.value)} />
                                ...
                                {sortedTasks?.map((task) => (
                                    <div key={task._id} className="p-4 hover:bg-muted/50">
                                        <div className="flex flex-col md:flex-row md:items-center justify-between mb-2 gap-3">
                                            <div className="flex">
                                                <div className="flex gap-2 mr-2 mt-0.5">
                                                    {task.status === "Done" ? <CheckCircle className="size-4 text-green-500" />
                                                        : <Clock className="size-4 text-yellow-500" />}
                                                </div>

                                                <div>
                                                    <Link to={`/workspaces/${task.project.workspace}/projects/${task.project._id}/tasks/${task._id}`}
                                                        className="font-medium hover:underline hover:text-primary transition-colors flex items-center"
                                                    >
                                                        {task.title}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <Card className="mb-4">
                            <CardHeader>
                                <CardTitle className="flex items-center justify-between">
                                    To Do
                                    <Badge variant="outline">{todoTasks?.length} tasks</Badge>
                                </CardTitle>
                            </CardHeader>

                            <CardContent className="p-3 space-y-3 max-h-150 overflow-y-auto">
                                {todoTasks?.map((task) => (
                                    <Card key={task._id} className="p-2 hover:shadow-md transition-shadow">
                                        <Link to={`/workspaces/${task.project.workspace}/projects/${task.project._id}/tasks/${task._id}`}
                                            className="block"
                                        >
                        ...
                        <Card className="mb-4">
                            <CardHeader>
                                <CardTitle className="flex items-center justify-between">
                                    In Progress
                                    <Badge variant="outline">{inProgressTasks?.length} tasks</Badge>
                                </CardTitle>
                            </CardHeader>
                        ...
                        <Card className="mb-4">
                            <CardHeader>
                                <CardTitle className="flex items-center justify-between">
                                    Done
                                    <Badge variant="outline">{doneTasks?.length} tasks</Badge>
                                </CardTitle>
                            </CardHeader>
